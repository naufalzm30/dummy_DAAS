  ...
  <div>
    ...
    <form @submit.prevent="update">
      <div class="mobile-width">
        ...
        <div class="row my-2">
          <div class="col-md subTitle">Edit Modem {{ item.modem_ver }}</div>
...
export default {
  ...
  methods: {
    async updateData() {
      await axios
        .put(
          `${this.$baseURL}/modem-type/` + this.$route.params.id,
          this.item,
          {
            headers: {
  ...
  async mounted() {
    await axios
      .get(`${this.$baseURL}/modem-type/` + this.$route.params.id, {
        headers: {
          Authorization: `Token ${this.token}`,
        },
      })
